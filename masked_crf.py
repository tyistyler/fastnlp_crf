...
class ConditionalRandomField(nn.Module):
    ...
    def __init__(self, num_tags, include_start_end_trans=False, allowed_transitions=None,
                 initial_method=None, is_masked_crf=False):
        """
        ...
        self.masked_training = is_masked_crf

        if is_masked_crf:
            self.trans_mask, self.start_trans_mask, self.end_trans_mask = self.get_mask_matrix(constrain, num_tags)

        initial_parameter(self, initial_method)

    def get_mask_matrix(self, constrain, num_tags):

        trans_mask = constrain[:num_tags, :num_tags]
        trans_mask = torch.where(trans_mask > -1, torch.tensor(100.0), trans_mask)
        trans_mask = nn.Parameter(trans_mask, requires_grad=False)

        start_trans_mask = constrain[num_tags:num_tags+1, :num_tags].view(num_tags)
        start_trans_mask = torch.where(start_trans_mask > -1, torch.tensor(100.0), start_trans_mask)
        start_trans_mask = nn.Parameter(start_trans_mask, requires_grad=False)

        end_trans_mask = constrain[:num_tags, num_tags+1:num_tags+2].view(num_tags)
        end_trans_mask = torch.where(end_trans_mask > -1, torch.tensor(100.0), end_trans_mask)
        end_trans_mask = nn.Parameter(end_trans_mask, requires_grad=False)

        return trans_mask, start_trans_mask, end_trans_mask

    def get_min_mask(self):

        masked_transitions = torch.min(self.trans_m, self.trans_mask)
        masked_start_transitions = torch.min(self.start_scores, self.start_trans_mask)
        masked_end_transitions = torch.min(self.end_scores, self.end_trans_mask)
    ...
    def _normalizer_likelihood(self, logits, mask):
        ...
        if self.masked_training:
            ...
            transitions, start_transitions, end_transitions = self.trans_m, self.start_scores, self.end_scores

        if self.include_start_end_trans:
            alpha = alpha + start_transitions.view(1, -1)

        flip_mask = mask.eq(0)

        for i in range(1, seq_len):
            emit_score = logits[i].view(batch_size, 1, n_tags)
            trans_score = transitions.view(1, n_tags, n_tags)
            tmp = alpha.view(batch_size, n_tags, 1) + emit_score + trans_score
            alpha = torch.logsumexp(tmp, 1).masked_fill(flip_mask[i].view(batch_size, 1), 0) + \
                    alpha.masked_fill(mask[i].eq(1).view(batch_size, 1), 0)

        if self.include_start_end_trans:
            alpha = alpha + end_transitions.view(1, -1)
    ...
    def _gold_score(self, logits, tags, mask):
        ...
        mask = mask.eq(1)
        flip_mask = mask.eq(0)
        trans_score = transitions[tags[:seq_len - 1], tags[1:]].masked_fill(flip_mask[1:, :], 0)
        # emit_score [L, B]
        ...
        if self.include_start_end_trans:
            st_scores = start_transitions.view(1, -1).repeat(batch_size, 1)[batch_idx, tags[0]]
            last_idx = mask.long().sum(0) - 1
            ed_scores = end_transitions.view(1, -1).repeat(batch_size, 1)[batch_idx, tags[last_idx, batch_idx]]
